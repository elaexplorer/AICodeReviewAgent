...
public class CodeReviewOrchestrator
{
        ...
        string codebaseContext)
    {
        ...
        // Validation pass — prune false positives and correct severities before returning
        if (allComments.Count > 0)
        {
            var beforeValidation = allComments.Count;
            allComments = await RunValidationPassAsync(allComments, files);
            _logger.LogInformation("🛡️  Validation pass: {Before} → {After} comments ({Dropped} dropped)",
                beforeValidation, allComments.Count, beforeValidation - allComments.Count);
        }
    ...
    /// <summary>
    /// Second-pass validator: reviews the full merged comment list against the actual diffs
    /// and prunes false positives, adjusts severities/confidence, and returns a pruned list.
    /// </summary>
    private async Task<List<CodeReviewComment>> RunValidationPassAsync(
        List<CodeReviewComment> comments,
        List<PullRequestFile> files)
    {
        const int MaxDiffCharsPerFile = 2_000;
        const int MaxCommentsToValidate = 60;

        try
        {
            _logger.LogInformation("🛡️  Starting validation pass for {Count} comments", comments.Count);

            // Build condensed diffs for context
            ...
            foreach (var file in files)
            {
                ...
                diffSb.AppendLine($"### {file.Path}");
                diffSb.AppendLine("```diff");
                diffSb.AppendLine(diff);
                diffSb.AppendLine("```");
            }

            // Serialize comments with index for the validator to reference
            var workingSet = comments.Take(MaxCommentsToValidate).ToList();
            var commentJson = System.Text.Json.JsonSerializer.Serialize(
                workingSet.Select((c, i) => new
                {
                    index = i,
                    filePath = c.FilePath,
            ...
            var diffs = diffSb.ToString();

            var prompt = $$$"""
                You are a senior code reviewer performing a VALIDATION pass.
                Below are candidate review comments generated by an AI reviewer, followed by the actual file diffs.

                Your task: for each comment, decide whether it is VALID or a FALSE POSITIVE.
                ...
                CANDIDATE COMMENTS (JSON array, each has an "index" field):
                {{{commentJson}}}
            ...
            {
                new(ChatRole.System, "You are a senior code reviewer performing a validation pass to prune false positives."),
                new(ChatRole.User, prompt)
            };

            _logger.LogInformation("📤 Validation prompt: {Chars} chars, validating {Count} comments",
                prompt.Length, workingSet.Count);
            ...
            _logger.LogInformation("📥 Validation response: {Chars} chars in {Ms}ms",
                responseText.Length, stopwatch.ElapsedMilliseconds);
            _logger.LogInformation("📝 Validation LLM response:\n{Response}", responseText);

            var pruned = ApplyValidationDecisions(workingSet, responseText);

            // Pass-through any comments beyond the validation cap unchanged
            if (comments.Count > MaxCommentsToValidate)
            {
                pruned.AddRange(comments.Skip(MaxCommentsToValidate));
                _logger.LogInformation("   {Count} comments beyond validation cap passed through unchanged",
                    comments.Count - MaxCommentsToValidate);
            }
        ...
        {
            _logger.LogError(ex, "Error in validation pass — returning original comment list");
            return comments;
        }
    }

    private List<CodeReviewComment> ApplyValidationDecisions(
        List<CodeReviewComment> comments,
        string jsonResponse)
    {
        try
        {
            ...
            var decisions = System.Text.Json.JsonSerializer.Deserialize<List<ValidationDecisionJson>>(jsonResponse, options)
                ?? [];

            var decisionMap = decisions.ToDictionary(d => d.Index);
            var result = new List<CodeReviewComment>();

            for (var i = 0; i < comments.Count; i++)
            {
                if (!decisionMap.TryGetValue(i, out var decision))
                {
                    // No decision returned for this index — keep as-is
                    result.Add(comments[i]);
                    continue;
                }

                if (string.Equals(decision.Decision, "drop", StringComparison.OrdinalIgnoreCase))
                {
                    _logger.LogInformation("   🗑️  Dropped comment #{Index} ({FilePath}:{Line}): {Reason}",
                        i, comments[i].FilePath, comments[i].StartLine, decision.Reason ?? "false positive");
                    continue;
                }

                // Keep — apply any severity/confidence adjustments
                var comment = comments[i];
                if (!string.IsNullOrEmpty(decision.Severity))
                    comment.Severity = decision.Severity.ToLower();
                if (decision.Confidence is > 0.0 and <= 1.0)
        ...
        {
            _logger.LogError(ex, "Error applying validation decisions — returning original comments");
            return comments;
        }
    }

    private class ValidationDecisionJson
    {
