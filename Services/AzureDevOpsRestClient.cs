...
public class AzureDevOpsRestClient
{
    ...
    private async Task<HttpResponseMessage> SendDeleteWithAuthFallbackAsync(
        string url,
        string? accessTokenOverride,
        string operationName)
    {
        ...
        request.Headers.Authorization = overrideHeader;
        var response = await _httpClient.SendAsync(request);
        if (!IsAuthFailureStatusCode(response.StatusCode))
            return response;

        _logger.LogWarning(
            "Bearer token authorization failed during {Operation}. Falling back to configured PAT.",
            operationName);
        response.Dispose();
        return await _httpClient.DeleteAsync(url);
    }
    ...
    public static string BuildCommentBody(CodeReviewComment comment)
    {
        ...
        var confidenceStr = comment.Confidence > 0.0 ? $" · confidence: {comment.Confidence * 100:F0}%" : "";
    ...
    /// <summary>
    /// Deletes all comment threads posted by this agent on the given PR.
    /// Identifies agent threads by either:
    ///   1. The footer marker "Generated by [CodeReviewAIAgent]" (user-token posts), or
    ///   2. Author is "Project Collection Build Service" AND content has our severity pattern
    ///      (pipeline-token posts).
    /// Returns the count of threads deleted.
    /// </summary>
    public async Task<int> DeleteAgentCommentsAsync(string project, string repositoryId, int pullRequestId, string? accessTokenOverride = null)
    {
        const string AgentMarker        = "Generated by [CodeReviewAIAgent]";
        const string PipelineAuthor     = "Project Collection Build Service";
        const string SeverityPattern    = "**[CRITICAL]";   // our comment format anchor
        const string SeverityPatternHigh = "**[HIGH]";
        const string SeverityPatternMed  = "**[MEDIUM]";
        const string SeverityPatternLow  = "**[LOW]";
        var deleted = 0;

        try
        {
            var url = $"https://dev.azure.com/{_organization}/{project}/_apis/git/repositories/{repositoryId}/pullRequests/{pullRequestId}/threads?api-version=7.1";
            var response = await SendGetWithAuthFallbackAsync(url, accessTokenOverride, "list threads");
            response.EnsureSuccessStatusCode();
            ...
            foreach (var thread in threads.EnumerateArray())
            {
                if (!thread.TryGetProperty("id", out var threadIdEl)) continue;
                var threadId = threadIdEl.GetInt32();

                if (!thread.TryGetProperty("comments", out var comments) || comments.ValueKind != JsonValueKind.Array)
                    continue;

                // Check if any comment in this thread was posted by our agent:
                // - via user PAT:      content contains our marker footer
                // - via pipeline token: author is Build Service AND content matches our severity format
                var isAgentThread = false;
                var firstCommentId = -1;
                foreach (var comment in comments.EnumerateArray())
                {
                    if (comment.TryGetProperty("isDeleted", out var del) && del.GetBoolean()) continue;

                    var text = comment.TryGetProperty("content", out var contentEl)
                        ? contentEl.GetString() ?? "" : "";

                    var authorName = "";
                    if (comment.TryGetProperty("author", out var authorEl) &&
                        authorEl.TryGetProperty("displayName", out var dnEl))
                        authorName = dnEl.GetString() ?? "";

                    var byMarker   = text.Contains(AgentMarker, StringComparison.Ordinal);
                    var byPipeline = authorName.Contains(PipelineAuthor, StringComparison.OrdinalIgnoreCase)
                                  && (text.Contains(SeverityPattern,     StringComparison.Ordinal)
                                   || text.Contains(SeverityPatternHigh,  StringComparison.Ordinal)
                                   || text.Contains(SeverityPatternMed,   StringComparison.Ordinal)
                                   || text.Contains(SeverityPatternLow,   StringComparison.Ordinal));

                    if (byMarker || byPipeline)
                        isAgentThread = true;

                    if (firstCommentId == -1 && comment.TryGetProperty("id", out var cidEl))
                        firstCommentId = cidEl.GetInt32();
                }

                if (!isAgentThread || firstCommentId == -1) continue;

                // Delete all comments in the thread (first comment last, as it anchors the thread)
                var commentIds = new List<int>();
                foreach (var comment in comments.EnumerateArray())
                {
                    if (comment.TryGetProperty("id", out var cidEl))
                        commentIds.Add(cidEl.GetInt32());
                }
                // Delete non-first comments first, then first
                foreach (var cid in commentIds.Where(id => id != firstCommentId))
                {
                    var delUrl = $"https://dev.azure.com/{_organization}/{project}/_apis/git/repositories/{repositoryId}/pullRequests/{pullRequestId}/threads/{threadId}/comments/{cid}?api-version=7.1";
                    await SendDeleteWithAuthFallbackAsync(delUrl, accessTokenOverride, $"delete comment {cid}");
                }
                var firstDelUrl = $"https://dev.azure.com/{_organization}/{project}/_apis/git/repositories/{repositoryId}/pullRequests/{pullRequestId}/threads/{threadId}/comments/{firstCommentId}?api-version=7.1";
                var delResp = await SendDeleteWithAuthFallbackAsync(firstDelUrl, accessTokenOverride, $"delete thread {threadId}");
        ...
        {
            _logger.LogWarning("DeleteAgentCommentsAsync failed for PR {PrId}: {Error}", pullRequestId, ex.Message);
        }
